...
#Reads from CAN and updtes the values in the GUI 
def speedometer_progress():
    ...
    elif msg.arbitration_id == 528 or msg.arbitration_id ==210: #MCU transmit request
        if int(msg.data.hex()[0:2],16) == 144: #Code for torque comand to MCU            
            if (msg.data.hex()[2:6],16)==0: 
                Tsal = Label(bg="green", width=200, height=5)#Sets TSAl to green
                Tsal.place(relx=0.5,rely=0.07, anchor="n")
            else:
                torqueCommand.set(int(abs(msg.data.hex()[2:6],16)-32766)/655.34)#Sets the turque command, sent to the ECU
                Tsal = Label(bg="red", width=200, height=5) #Sets TSAL to red 
                Tsal.place(relx=0.5,rely=0.07, anchor="n")
